...
import 'package:work_tracker/features/work_item/domain/models/work_item_time_log.dart';
...
class FakeTaskDao implements WorkItemDao {
  final Map<int, WorkItemEntity> _store = {};
  int _nextId = 1;
  ...
  @override
  void update(WorkItemEntity entity) => _store[entity.id] = entity;

  @override
  List<WorkItemEntity> getAll() => _store.values.toList();

  @override
  WorkItemEntity? getById(int id) => _store[id];
...
/// `WorkItemRepositoryImpl` actually calls. The rest throw if ever hit, so a test
...
class FakeTaskTimeLogRepository implements WorkItemTimeLogRepository {
  ...
  @override
  Future<List<WorkItemTimeLog>> getAll() => throw UnimplementedError();
...
void main() {
  ...
  late WorkItemRepositoryImpl repository;

  setUp(() {
    dao = FakeTaskDao();
    zentaoRepository = FakeZentaoRepository();
    timeLogRepository = FakeTaskTimeLogRepository();
    repository = WorkItemRepositoryImpl(dao, zentaoRepository, timeLogRepository);
  });
  ...
  group('importFromZentao', () {
    test('maps a ZentaoTask into a linked WorkItem', () async {
  ...
  group('generic external identity (multi-platform)', () {
    test('a manual task maps to source manual with no external id', () async {
      ...
      expect(task.source, WorkItemSource.manual);
    ...
    test('a legacy zentao bug row (no stored source) derives zentao/bug', () async {
      ...
      expect(task.source, WorkItemSource.zentao);
  ...
  group('sync-driven completion (Zentao resolved/closed -> done)', () {
    ...
    test('two-way: an active re-sync un-marks an already-done task, keeping '
        'tracked time', () async {
      await repository.upsertBugFromZentao(
        const ZentaoBug(id: 7, title: 'Crash', status: 'active'),
      );
      ...
      // The bug was reopened on the server (active again) -> the local task
      // returns to not-done, but its tracked time is preserved.
      await repository.upsertBugFromZentao(
        const ZentaoBug(id: 7, title: 'Crash', status: 'active'),
      );
      final refreshed = dao.findByZentaoBugId(7)!;
      expect(refreshed.done, isFalse);
      expect(refreshed.elapsedSeconds, 120);
    });
