...
# Same reason as scripts/build_release_apk.sh: `GOOGLE_MAPS_API_KEY` is needed
# in two different places for a release build — baked into the Dart binary via
# `--dart-define-from-file` (read via `String.fromEnvironment` in
# lib/di/register_module.dart), and injected into AndroidManifest.xml via a
# Gradle manifest placeholder that reads `android/local.properties` (see
# android/app/build.gradle.kts). This script treats the repo-root dart_defines
# file as the single source of truth and syncs its GOOGLE_MAPS_API_KEY into
# android/local.properties before every release build.
#
# The Play Store requires an .aab (App Bundle), not an .apk — this script is
# the AAB counterpart of build_release_apk.sh, used by the `internal`/`closed`
# fastlane lanes (see docs/play_store_setup.md).
#
# See docs/leave_reminder_setup.md for how to provision the dart_defines file.
#
# Usage: ./scripts/build_release_aab.sh
# Optional: BUILD_NUMBER=<n> to override the version code (the `closed`/`internal`
# fastlane lanes set this from the unified Play/Firebase counter — see
# docs/versioning.md and docs/play_store_setup.md), BUILD_NAME=<x.y.z> to
# override the version name (normally left unset so it comes from pubspec.yaml),
# DART_DEFINES_FILE=<file> for a tier-specific config (prod builds use
...
if [[ ! -f "$DART_DEFINES_FILE" ]]; then
  ...
  echo "GOOGLE_MAPS_API_KEY before building a release AAB." >&2
  exit 1
fi
...
fvm flutter build appbundle --release --flavor "$FLAVOR" --dart-define-from-file="$DART_DEFINES_FILE" $BUILD_NAME_ARG $BUILD_NUMBER_ARG
