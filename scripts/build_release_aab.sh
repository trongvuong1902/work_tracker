#!/usr/bin/env bash
#
# Build a release AAB (Android App Bundle) for Google Play Store uploads.
#
...
# Optional: BUILD_NUMBER=<n> ./scripts/build_release_aab.sh to override the
# version code (used by the `internal` fastlane lane to keep the Play Store
...
fvm flutter build appbundle --release --flavor "$FLAVOR" --dart-define-from-file="$DART_DEFINES_FILE" "${BUILD_NUMBER_ARGS[@]}"
