...
import 'package:work_tracker/features/work_item/presentation/widgets/resolve_duration_sheet.dart';
...
/// WorkItem Detail: title, description, attachments and notes as stacked tiles,
...
class WorkItemDetailPage extends StatelessWidget {
  const WorkItemDetailPage({super.key, required this.taskId});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => getIt<WorkItemDetailCubit>()..load(taskId),
      child: const _WorkItemDetailView(),
    );
  }
}

class _WorkItemDetailView extends StatefulWidget {
  const _WorkItemDetailView();

  @override
  State<_WorkItemDetailView> createState() => _WorkItemDetailViewState();
}

class _WorkItemDetailViewState extends State<_WorkItemDetailView> {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<WorkItemDetailCubit, WorkItemDetailState>(
        ...
        return Scaffold(
          ...
          bottomNavigationBar: task == null
              ...
              : _TimerBar(
                  ...
                  onToggle: context.read<WorkItemDetailCubit>().toggleTimer,
                ),
        );
      },
    );
    ...
    return ListView(
      ...
      children: [
        ...
        _PlanDateControl(
          ...
          onClear: () => context.read<WorkItemDetailCubit>().setPlannedDate(null),
        ),
        if (isBug) ...[
          const SizedBox(height: AppSpacing.space12),
          _BugStatusControl(
  ...
  void _showInfoSheet(
    ...
    final cubit = context.read<WorkItemDetailCubit>();
    showModalBottomSheet<void>(
      context: context,
      showDragHandle: true,
      builder: (_) => BlocProvider.value(
        value: cubit,
        child: _WorkItemInfoSheet(connection: _zentaoConnection),
      ),
    );
  }

  Future<void> _pickPlannedDate(BuildContext context, WorkItem task) async {
    final cubit = context.read<WorkItemDetailCubit>();
  ...
  Future<void> _showStatusSheet(BuildContext context, WorkItem task) async {
    final cubit = context.read<WorkItemDetailCubit>();
    ...
    // Resolving a bug: confirm the time to report first, pre-filled from the
    // tracked total. Cancelling the duration sheet aborts the resolve.
    int? durationMinutes;
    if (action == BugStatusAction.resolve) {
      if (!context.mounted) return;
      durationMinutes = await showResolveDurationSheet(
        context,
        suggestedMinutes: task.currentElapsedSeconds() ~/ 60,
      );
      if (durationMinutes == null) return;
    }

    await cubit.changeBugStatus(action, durationMinutes: durationMinutes);
    if (!mounted) return;
  ...
  void _showClaudePromptSheet(BuildContext context, WorkItem task) {
    showModalBottomSheet<void>(
      context: context,
  ...
  void _resolveWithAi(BuildContext context, WorkItem task) {
...
List<BugStatusAction> _statusActionsFor(WorkItem task) {
...
/// Open-in-Zentao.
class _WorkItemInfoSheet extends StatelessWidget {
  const _WorkItemInfoSheet({required this.connection});

  final ZentaoConnection? connection;

  @override
  Widget build(BuildContext context) {
    return BlocConsumer<WorkItemDetailCubit, WorkItemDetailState>(
  ...
  Future<void> _openInZentao(WorkItem task, ZentaoConnection connection) async {
