...
import '../../../../../work_item/domain/work_item_repository.dart';
import '../../../../../work_item/domain/work_item_time_log_repository.dart';

part 'today_work_items_state.dart';
part 'today_work_items_cubit.freezed.dart';
...
@injectable
class TodayWorkItemsCubit extends Cubit<TodayWorkItemsState> {
  TodayWorkItemsCubit(this._taskRepository, this._timeLogRepository)
    : super(const TodayWorkItemsState());

  final WorkItemRepository _taskRepository;
  final WorkItemTimeLogRepository _timeLogRepository;

  Map<int, int> _loggedToday = {};
  List<WorkItem> _tasks = const [];
  Timer? _ticker;
  ...
  Future<void> init() async {
    ...
    // the WorkItem Detail or Manage task times pages) — HomePage stays alive in an
  ...
  void _emitItems() {
    ...
    emit(TodayWorkItemsState(isLoading: false, items: items));
  }
